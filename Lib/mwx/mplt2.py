...
class ParamCtrl(wx.Panel):
    ...
    param : A param <Param> object referred from knobs <ParamCtrl>
  ...
  lw, tw, cw : width of label, textbox, and control (default height `h=22 of widgets)
    ...
    def OnCtrlKeyDown(self, evt): #<wx._core.KeyEvent>
        ...
        def focus(c):
            if isinstance(c, ParamCtrl) and c.ctrl.IsEnabled():
                c.ctrl.SetFocus()
                return True
...
class ControlPanel(scrolled.ScrolledPanel):
    ...
    def Destroy(self):
        for k in chain(*self.__groups):
            if isinstance(k, ParamCtrl):# ParamCtrl: パラメータの関連付けを解除する
                k.Destroy()
  ...
  [hv]spacing : spacing among packed objs inside the group
     ...
     **kwargs : extra keyword arguments given for ParamCtrl
        """
        ## assert all((key in inspect.getargspec(ParamCtrl)[0]) for key in kwargs)
        ...
        objs = [ (c, 0, wx.EXPAND) if isinstance(c, wx.StatusBar)
            ...
            else ParamCtrl(self, c, **kwargs) for c in objs ]
        
        self.__groups.append([c for c in objs if isinstance(c, wx.Object)])
        
        def isvar(c):
            return c.param if isinstance(c, ParamCtrl)\
...
if __name__ == '__main__':
    ...
    class TestPanel(ControlPanel, mwx.CtrlInterface):
        def __init__(self, *args, **kwargs):
            ...
            self.layout("types", (
                ParamCtrl(self, self.A, type, lw=32, tw=60, cw=0, h=20)
                for type in (
                    None,
                    'vspin',
                    'hspin',
